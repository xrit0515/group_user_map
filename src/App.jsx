    ...
    async function loadChinaMap() {
      ...
      for (const url of MAP_JSON_SOURCES) {
        try {
          ...
          // Log all region names from the map
          const regionNames = geoJson.features?.map(f => f.properties?.name).filter(Boolean) || [];
          console.log('All region names from map:', regionNames);
          setMapLoaded(true);
          return;
        } catch (err) {
